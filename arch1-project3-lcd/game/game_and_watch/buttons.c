...
extern unsigned char fallingObject;
...
void __attribute__((interrupt(PORT2_VECTOR))) Port_2() {
    ...
    } else if (P2IFG & BIT3) { // Button S4 pressed (Reset Falling Object)
        fallingObject = (fallingObject + 1) % 3; // Cycle falling object position
